...
p <- prior(normal(0, .5), class = b, resp = val) +
  prior(normal(0, 1), class = b, coef = sex1, resp = val) +
  # Standard deviations
  prior(
    student_t(7, 0, .5), 
    class = sd, resp = val, group = region
  ) +
  prior(
    student_t(7, 0, 1.5), 
    class = sd, resp = val, group = region, coef = Intercept
  ) +
  prior(
    student_t(7, 0, .5), 
    class = sd, resp = val, group = country
  ) +
  prior(
    student_t(7, 0, 1.5), 
    class = sd, resp = val, group = country, coef = Intercept
  ) +
  # Correlations
  prior(lkj(2), class = cor, group = country) +
  prior(lkj(2), class = cor, group = region)
...
# Regression coefficients
p <- prior(normal(0, .5), class = b) +
  prior(normal(0, 1), class = b, coef = sex1) +
  # Standard deviations
  prior(
    student_t(7, 0, .5), class = sd, group = region
  ) +
  prior(
    student_t(7, 0, 1.5), 
    class = sd, group = country, coef = Intercept
  ) +
  prior(
    student_t(7, 0, .5), class = sd, group = region
  ) +
