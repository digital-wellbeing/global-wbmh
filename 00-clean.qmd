...
gwp <- gwp %>%
  ...
  transmute(
    ...
    across(wp60:wp74, ~ 3 - .),
    # Internet (fix NAs and reverse 1:yes 2:no coding to 1:no 2:yes)
    gwpi = 2 - if_else(between(wp16056, 1, 2), wp16056, NaN)
  )
...
gwp <- gwp %>%
  ...
  mutate(gwpi = mean(gwpi, na.rm = TRUE)) %>% 
  pivot_longer(
    c(
      Life_satisfaction,
      Negative_experiences,
  ...
  group_by(country, year, sex, age, outcome, gwpi) %>%
  summarise(
...
if (!file.exists(gwp_mock_path)) {
  set.seed(101010)
  distinct(gwp, outcome, country, year, sex, age, gwpi) %>% 
    mutate(
      se = rnorm(
        nrow(.), 
        mean(gwp$se, na.rm = TRUE), 
        sd(gwp$se, na.rm = TRUE)
      ),
      val = rnorm(
        nrow(.), 
        mean(gwp$val, na.rm = TRUE), 
        sd(gwp$val, na.rm = TRUE)
      )
    ) %>% 
    write_rds(gwp_mock_path, compress = "xz")
}
...
```{r gwp-summary-table}
#| column: page-right
#| tbl-cap: "Mean internet user percentages across regions and time from the GWP data."
#| label: tbl-gwpsummary
...
gwp %>%
  ...
  ungroup() %>% 
  mutate(gwpi = gwpi * 100) %>%
  mutate(region = str_replace(region, "_", " ")) %>%
  arrange(year, region, country) %>% 
  pivot_wider(names_from = year, values_from = gwpi) %>%
  select(-country) %>%
  tbl_summary(
    by = region,
    missing = "no",
  ...
  add_overall(col_label = "Total") %>%
  as_kable_extra(
    caption = "GWP Internet use summary (mean over demographics)"
  ) %>%
  row_spec(0, bold = TRUE) %>%
  kable_custom()
```
