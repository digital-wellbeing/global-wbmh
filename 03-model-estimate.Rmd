...
# when this is used as an outcome
itu <- itu %>% 
  mutate(ib = if_else(internet == 0, .0000001, internet)) %>% 
  mutate(ib = if_else(internet == 1, .9999999, internet))
...
```{r}
# Give each outcome value a tag to ensure it is used in MV model
dat <- dat %>% 
  mutate(y = !is.na(val))

# Indicate unique ITU values for MV model
dat <- dat %>% 
  # Unique values exist for each country-year
