---
title: Global mental health and well-being in the internet age
description: | 
  Online analysis supplement to "Global mental health and well-being in the internet age (Vuorre & Przybylski, 2022)"
author:
  - name: Matti Vuorre
...
tab %>% 
  ...
  mutate(
    ...
    scaled_result = str_glue(
      "{number(scaled_mean, .001)} 
      [{number(scaled_low, .001)}, 
      {number(scaled_high, .001)}] 
      ({percent2(rope, .1)})"
    ),
...
cor_sum %>% 
  ...
  mutate(
    across(Mean:CI_high, ~number(., .01)), pd = percent2(pd, .1)
  ) %>% 
  transmute(
