...
# Assign N of 1 cells a standard deviation of zero
gwp <- gwp %>% 
...
\begin{align*} 
y_i &\sim \text{Normal}(\mu_i, \sqrt{\sigma^{2} + \text{sd}^2_i}), \\
...
where $i$ indexes rows in the data and $\text{sd}_i$ are the known standard deviations. $\text{X}^{\text{CW}}_i$ and $\text{X}^{\text{CB}}_i$ are the within- and between-country centred internet technology adoption predictor (per capita internet users or mobile broadband subscriptions, in separate models), respectively. In addition to the standard deviations, we weighted each observation by the number of respondents in that cell, to adjust for varying sample sizes across cells. We used default noninformative priors on all parameters.
