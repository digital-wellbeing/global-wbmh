...
p1 <- itu %>%
  ...
  mutate(
    Mobile = if_else(Mobile > 150, NaN, Mobile),
    ) %>%
  pivot_longer(
    c(Internet, Mobile),
    names_to = "outcome", values_to = "val"
  ...
  filter(region != "Oceania", year >= 2005) %>% 
  filter(outcome %in% c("Internet", "Mobile", "Life_satisfaction")) %>% 
  mutate(year = make_date(year)) %>%
  mutate(country = fct_relevel(country, "Netherlands") %>% fct_rev()) %>%
  ggplot(
    aes(
      year, 
      mean, 
      size = country == "Netherlands", 
      color = region == "Europe"
    )
  ) +
...
p2 <- x %>% 
  ...
  ggplot(aes(value, fct_rev(Predictor), fill = x, color = x)) +
  scale_color_manual(
    "Association", 
    values = c("#fc8d62", "#66c2a5"), 
    aesthetics = c("fill", "color", "interval_color")
  ) +
...
p_country_wb <- figure_country(tab_wb %>% filter(outcome == "Life_satisfaction"))
p_country_wb
```
