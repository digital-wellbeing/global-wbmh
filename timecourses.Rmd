...
Our first research question concerns changes in mental health and well-being over time: To what extent has adolescent mental health changed over time? And, do changes in well-being correlate with changes in internet adoption? To answer, we examine Model 1's results.
...
tmp <- itu %>% 
  ...
  group_by(country) %>% 
  mutate(i = internet - lag(internet)) %>% 
  summarise(i = mean(i, na.rm = TRUE))
internet_yoy <- percent(mean(tmp$i, na.rm = TRUE), .1)
```

Average year-on-year increase in internet adoption was `r internet_yoy`.

## Main result: Trends

```{r}
get_tab <- function(pattern, label, sort = outcome) {
  fits %>% 
    mutate(
      out = map(
        fit1,
    ...
    arrange({{sort}}) %>% 
    kbl(digits = 2, caption = label) %>% 
    kable_custom()
}

get_tab(
  "b_val_(Intercept|year)$", 
  "Model 1: Population level effects",
  variable
)
```

This table shows the population-level (average across region, country, age) intercepts and time-courses. Negative experiences had increased. We then summarise the region- and country-level trends in a table.

```{r}
get_coeftab <- function(x, group) {
  coef(x, summary = FALSE)[[group]][,,"val_year"] %>% 
    summarise_draws(
      mean, 
      ~quantile2(., probs = c(.025, .975)),
      pd = ~pd(as.numeric(.)), 
...
val_year_region <- fits %>% 
  mutate(out = map(fit1, ~get_coeftab(.x, "region"))) %>% 
  select(outcome, out)
val_year_country <- fits %>% 
  mutate(out = map(fit1, ~get_coeftab(.x, "country"))) %>% 
  select(outcome, out)
  
bind_rows(
  "region" = val_year_region,
  "country" = val_year_country,
  .id = "Grouping"
) %>% 
  unnest(out) %>% 
  rename(Group = variable) %>% 
  kbl(digits = 2, caption = "Country-specific timecourses") %>% 
  kable_custom() %>% 
  scroll_box("500px")
```
