...
file_path <- "brm-time-main-effects"
if (!file.exists(file_path)) {
  job::job({
    fit_anx <- brm(
  ...
  write_rds(fits_brm_time_main, file_path)
} else {fits_brm_time_main <- read_rds(file_path)}
```
...
# Fit models
file_path <- "brm-time-interactions"
if (!file.exists(file_path)) {
  job::job({
    fit_anx <- brm(
      bf_int + bf_anx + set_rescor(FALSE),
  ...
  write_rds(fits_brm_time_interactions, file_path)
} else {fits_brm_time_interactions <- read_rds(file_path)}
```
