...
itu %>% 
  ...
  ggplot(aes(x = year, y = internet, group = country)) +
...
ggsave(
  "output/Fig-ITU.png", 
  device = agg_png,
  height = 1400, width = 3200, res = 300, units = "px"
)
```
...
Here we create two plots. One to show all main variables' time trends for each region, and country within region. Then, show the outcome time trends for top internet trend in each region.
...
pa <- bind_rows(
  ...
  itu %>% 
    ...
    summarise(mean = mean(internet, na.rm = TRUE)) %>% 
    mutate(cause = "Internet")
) %>% 
  ungroup() %>% 
  mutate(cause = fct_relevel(cause, "Internet")) %>% 
  mutate(year = make_date(year)) %>% 
  ggplot(aes(year, mean, color = cause)) +
  scale_color_brewer(
    "Variable",
  ...
  geom_line(aes(group = interaction(country, cause)), alpha = .25, size = .4) +
  stat_summary(fun = mean, geom = "line", size = .6) +
  facet_grid(cause~region, scales = "free_y") +
  theme(legend.position = "none")
...
# Calculate random demographics time effects
random_effects %>% 
  mutate(
    `Male (20 to 24)` = time + `time:sex1`*.5 + `time:sex1:age1`*.25
  )

pivot_longer(
    c(`(Intercept)`:`time:sex1:age1`, `Male (20 to 24)`:`Female (15 to 19)`), 
    names_to = "Parameter", 
...
bind_rows(
  ...
  geom_point(
    data = random_effects %>% 
      filter(cluster == "country"), 
    shape = 1, size = .15, alpha = .2,
...
```{r fit-gbd-time-brms, eval = FALSE}
# bf_gbd_time <- bf(
#   mvbind(
#     val_Depression,
#     val_Anxiety,
...
p <- prior(normal(0, 1), class = b, resp = Anxiety) +
  prior(normal(0, 1), class = b, resp = Depression) +
  prior(normal(0, 1), class = b, resp = Selfharm) +
  prior(student_t(7, 0, .5), class = sd, resp = Anxiety) +
  prior(student_t(7, 0, .5), class = sd, resp = Depression) +
  prior(student_t(7, 0, .5), class = sd, resp = Selfharm)

fit_gbd_time_demos <- brm(
  bf_gbd_time_demos,
  gbd_y_w,
  prior = p,
  file = "output/brm-gbd-time-mv-demos",
  chains = 5,
  cores = 5,
  iter = 250, warmup = 150, refresh = 10,
  threads = 2,
  control = list(adapt_delta = .9, max_treedepth = 15)
)
```
