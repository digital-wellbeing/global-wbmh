...
Average well being and internet in each country

```{r}
x <- d %>% 
  mutate(outcome = if_else(str_detect(outcome, "_"), "Well-being", outcome)) %>% 
  filter(outcome %in% c("Well-being", "Internet")) %>% 
...
map_internet <- world %>% 
  mutate(country = countryname(name_long)) %>% 
  inner_join(x) %>% 
  tm_shape() +
  tm_borders(lwd = .15)
...
tmap_arrange(pa, pb, sync = TRUE)
```

## GBD metrics

While the Gallup data are (intended to be) representative of each country & year, there is more uncertainty in the GBD data. The latter are meta-analytic estimates produced by the IHME and as such can be inaccurate in some locations and or/years. Here, we compare the GBD self-harm indicator to CDC's self-injury data from <https://wisqars.cdc.gov>.

```{r}
# Load data sets
gbd <- read_rds("data/gbd.rds") %>% 
  filter(country == "US", outcome == "Selfharm") %>% 
  group_by(year, age) %>% 
  summarise(val = mean(val), se = mean(se)) %>% 
  filter(year > 2001)

cdc <- read_csv("data-raw/CDC/wisqars.csv") %>% 
  clean_names()
cdc <- cdc %>% 
  mutate(
    val = as.numeric(injuries) / as.numeric(population),
    se = as.numeric(standard_error) / as.numeric(population)
  ) %>% 
  select(
    year, 
    age = age_group,
...
bind_rows(
  ...
  drop_na(val) %>% 
  ggplot(aes(year, val, col = Source, fill = Source, group = Source)) +
  scale_y_continuous(
    "Rate",
    labels = ~percent(., .01),
    breaks = extended_breaks(5)
  ) +
  geom_ribbon(aes(ymin = val - se, ymax = val + se), col = NA, alpha = .2) +
  geom_line() +
  facet_wrap("age", ncol = 8) +
  theme(
