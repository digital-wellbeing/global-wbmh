...
-   Combine all predictors into one table (primary ITU and controls HDI)
...
itu <- itu %>%
  group_by(country) %>%
  mutate(
    # Year-specific deviations from country average
    i_cw = internet - mean(internet, na.rm = TRUE), 
    m_cw = internet - mean(mobile, na.rm = TRUE),
    h_cw = internet - mean(hdi, na.rm = TRUE),
    i1_cw = i1 - mean(i1, na.rm = TRUE), 
    m1_cw = m1 - mean(m1, na.rm = TRUE),
    h1_cw = h1 - mean(h1, na.rm = TRUE)
  ) %>% 
  ungroup()
