...
gwp <- gwp %>% 
  ...
  transmute(
    ...
    `Life\nsatisfaction` = if_else(between(wp16, 0, 10), wp16, NaN),
...
gwp <- gwp %>% 
  pivot_longer(
    ...
    names_to = "cause", values_to = "val"
  ) %>% 
  group_by(country, year, sex, age, cause) %>% 
  summarise(
...
gwp <- gwp %>% 
  mutate(
    country_harmonised = countryname(
      country, destination = "cldr.short.en"
    )
  ) %>% 
...
distinct(gwp, country) %>% filter(is.na(country))
```
...
distinct(gbd, country) %>% filter(is.na(country))
```
...
distinct(itu, country) %>% filter(is.na(country))

# These countries don't exist in outcomes
itu %>%
  distinct(country) %>% 
  mutate(gwp = country %in% unique(gwp$country)) %>% 
  mutate(gbd = country %in% unique(gbd$country)) %>% 
  filter(!gwp | !gbd) %>% 
  kbl() %>% 
  kable_minimal(full_width = FALSE) %>% 
  scroll_box(width = "500px", height = "400px")
...
# Drop them
itu <- itu %>% 
  filter(country %in% unique(gbd$country) | country %in% unique(gwp$country))
```
...
write_rds(gwp_all, "data/gwp_all_ages.rds")
